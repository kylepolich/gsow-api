...
p = PageviewsClient(user_agent="<kyle@dataskeptic.com> Statistical analysis")

def run_update():
  # TODO: get list of pages being watched
  # TODO: for each page, check where it left off last
  # TODO: query for updates
  # TODO: handle failures elegantly
  # TODO: save API results to database
  # TODO: update bookmark for next run
  pass


def get_updates_for_page_id(lang, pageid, start):
  s = start.replace('-', '') + "00"
  now = str(datetime.now())[0:10]
  e = now.replace('-', '') + "23"
  r = p.article_views(f'{lang}.wikipedia', [pageid], granularity='daily', start=s, end=e)  
  owner = 'wiki-pageviews'
  stream_id = f'{owner}.{lang}.{pageid}'
  items = []
  for k in r.keys():
      v = r[k]
      ts = int(k.timestamp())
