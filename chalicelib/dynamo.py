...
class DynamoDocstore(object):
    ...
    def get_document(self, object_id, initialize=False):
    ...
    def get_list(self, search_value, limit=None):
        owner_index = 'ownerIndex'
        attribute = 'owner'
        return self.owner_query(owner_index, attribute, search_value, limit)



    def get_batch_documents(self, object_ids):
        if len(object_ids) < 100:
            return self._get_batch_documents_1kmax(object_ids)
        else:
            result = []
            lst = list(object_ids)
            while len(lst) > 0:
